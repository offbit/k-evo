...
class TournamentOptimizer:
    ...
    def __init__(self, population_sz, init_fn, nb_workers=2, use_cuda=True):
        """
        ...
        self.population = [init_fn(i, 28*28, 10, 3) for i in range(population_sz)]        
    ...
    def step(self):
        ...
        for i in range(0,self.population_sz,2):
            print(len(self.population[i].nodes))
        self.evaluate()
        children = []
        ...
        while len(children) < self.population_sz:
            ...
            best = rank[pick][0]
            model = copy.deepcopy(self.population[best])
            children.append(model)
            children[-1].mutate_net()
